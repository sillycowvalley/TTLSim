...
U20, and a small link field — generic through subtraction, per house policy.
...
- **No instruction decode.** A four-bit *function code* arrives, plus the C
  ...
  published table of the JEDEC burned into U20, and nothing else.
...
1. In active-high data mode the '181's Cn and Cn+8 are active-low. Cn is
   driven by U20, so its inversion is written into the GAL equation and
...
inversion lives in a GAL equation — now U20's, on the module, rather than
GAL3's on the CPU board.

---

## The function-code GAL (U20, GAL16V8)
...
**Per-board, identical JEDECs.** A 16-bit pair fits U20 on both boards with
...
power the board with U20's socket empty — S, M, and Cn float.
...
```
  ...
  CFLG ───►│ U20 GAL16V8  │  Cn (direct) ►│ U1, U2    │──► Cn+4 ─► cond. ─► HCOUT
           ...
           └──────────────┘                └─────┬─────┘◄── LK_CN: GAL Cn / CNX (seam)
                                                 │ F (LS levels)
                                        ┌────────▼────────┐
                                        │ U9 74HCT541     │   the level fence
                                        │ always enabled  │   one per board
                                        └──┬────┬────┬────┘
                ┌──────────────────────────┘    │    └───────────────────┐
                │                               │                        │
          F port (H8/H9)                 U10 '688 ─► ZERO       U7 '541: FB → D (/FOE)
          always driven,                 FB7 ──────► SIGN       U8 '541: FB>>1 → D (/SOE)
          HC levels                      FB0 ──────► SOUT              SRIN ─► bit 7
                │
                └─► U16 V logic ('86 + AND) ─► V        U14/U15 '377 flag latch (option)
                                                        U17 '280 parity (option)
...
| Ref | Part | Role |
|-----|------|------|
| U1, U2 | 74LS181 | ALU slices, low and high nibble |
| U3 | 74S182 | carry lookahead across the two slices — fitted as standard (stock is plentiful; TTLSim has a '182 model) |
| U5, U6 | 74HC573 **or** 74HC574 | A and B operand latches — pin-compatible pair; transparent or edge per host |
| U9 | 74HCT541 | **the level fence** — F at LS levels → FB at HC levels, permanently enabled |
| U10 | 74HC688 | zero detect on FB, cascade via /ZCASC |
| U12 | 74HCT04 | Cn+4 / Cn+8 / A=B normalisation (HCT thresholds on LS outputs) |
| U13 | 74HCT86 | LK_BORROW conditioning on COUT/HCOUT; two gates spare |
| U19 | 74HC00 | glue: bus interlock, zero-cascade conditioning, spare AND for V |
...
|-----|------|------|-----------|
| U7 | 74HC541 | FB → D bus, /FOE | bus machines |
| U8 | 74HC541 | FB shifted right → D bus, /SOE; crossed inputs, bit 7 ← SRIN | 8080-ish, 6502-ish |
| U16 | 74HC86 | V (signed overflow), sign-bit formulation, + one AND from U19 | Addy, 6502-ish |
| U17 | 74HC280 | even parity over FB[7:0] | 8080-ish |
| U14, U15 | 74HC377 | latched flags, two independent enable domains (C-domain / NZ-domain) | hosts not owning their flags |
...
Convention from the clock module: **GND on pin 1 of every header, VCC on the
power header only, 8-pin maximum**. Spare pins on byte-port headers wired to
GND (ribbon return pattern). No series elements at any header — labels keep
one name across the boundary; TTL014 does not apply.

| Hdr | Pins | Contents |
|-----|------|----------|
| H1 | 2 | 1 GND · 2 VCC |
| H2/H3 | 8 | A operand: 1 GND · 2–5 A0–A3 (A4–A7) · 6–8 GND |
| H4/H5 | 8 | B operand: same pattern |
| H6/H7 | 8 | D bus (option): same pattern |
| H8/H9 | 8 | F result: same pattern |
| H10 | 8 | 1 GND · 2–5 **FC0–FC3** · 6 **CFLG** · 7 /LEA · 8 /LEB |
| H11 | 8 | 1 GND · 2 /FOE · 3 /SOE · 4 SRIN · 5–8 GND |
| H12 | 8 | 1 GND · 2 COUT · 3 HCOUT · 4 ZERO · 5 SIGN · 6 SOUT · 7 AEQB · 8 V |
| H13 | 8 | 1 GND · 2 /G · 3 /P · 4 **/CN8** · 5 **CNX** · 6 /ZCASC · 7 /AUXOE · 8 PAR |
| H14 | 8 | 1 GND · 2 CLK · 3 FLGC · 4 FLGNZ · 5 CLAT · 6 ZLAT · 7 NLAT · 8 VLAT |

### Control inputs

| Signal | Sense | Meaning |
|--------|-------|---------|
| FC3–FC0 | code | the operation, per U20's published JEDEC table |
...
routes U20's SUB output into the COUT/HCOUT XORs for hosts whose C flag is a
...
LK_CN disconnects U20's Cn output in favour of CNX; everything else about
...
/P come out on H13 for a second-level '182 on the host or a small carry
...
|-----|--------------------|----------|----------|-------|
| U1–U3 '181 ×2 + '182 | ● | ● | ● | ● |
| U20 JEDEC | `ALU_ADDY` (both boards) | `ALU_8080` | `ALU_6502` | host JEDEC under test |
| U5, U6 latches | **'574** (edge — the '670 write-through fence; replaces the CPU board's 4× '574) | '573 transparent | '573 or strap-through | strap-through |
| U7 D-bus driver | — (F port → register file D) | ● | ● | — |
| U8 SHR driver | — | ● | ● | — |
| U16 V | ● (high board) | — | ● | — |
| U17 parity | — | ● | — | — |
| U14/U15 flag latch | — (flags live in the CPU's '74/'157 block) | ● or host-side | ● or host-side | — |
| U18 aux GAL | — | ● (DAA) | — | — |
...
byte lane is correct — 2 for 16 bits. Carried into this design as U9 per
board.
...
|----------|-------------|------|------|
| U3 '182 | ripple ('04 sections between slices) | none remaining — TTLSim has a '182 model and stock is plentiful | ~2 extra carry hops; recorded only so it isn't re-proposed |
| U5/U6 '573 | '574 | edge-triggered — required where the operand source has a write-through hazard (Addy's '670) | needs a real clock edge from the host, not a level |
| U9 HCT541 | HCT245 (DIR strapped) | same job | control pins to strap; no gain — **don't**, and never with /OE grounded on a shared bus (bus-contention trap, see IO module U1 note) |
...
| U12/U13 HCT | HC | — | **fails** — these read LS outputs; VIH not met. The classic works-on-bench intermittent. Must be HCT |
| U17 '280 | 2× '86 tree | no '280 sourcing | two packages and a slower tree for the same flag |
| U20 GAL16V8 | GAL22V10 | more inputs/terms if a host outgrows 16V8 | bigger part for no current need; footprint is 20-pin — a 22V10 is a respin, so confirm the 16V8 budget per JEDEC before layout |
...
2. Clock contract only if U14/U15 or edge latches are fitted: GND, +5, CLK
...
3. Operands to H2–H5, FC and CFLG to H10, conditions from H12. F from H8/H9
   **or** the D bus via H6/H7 — a register-file machine (Addy) uses F and
...
FC[3:0], and CFLG; LEDs on F and H12.
...
3. **Bench: walk the FC table.** Every code in the fitted JEDEC, checked
   ...
   crossed nibble shows immediately; probe U20's output pins directly on
...
control path (FC → U20 → S/M/Cn) settling in parallel from the earlier IR
edge — except **Cn from CFLG, which is in series**: CFLG → U20 → '181, so
the GAL delay lands inside the arithmetic setup on ADC/SBC-class codes.

| Path | Estimate |
|------|----------|
| '573 through | ~20 ns |
| U20 GAL16V8 (FC/CFLG → S, M, Cn) | ~10 ns |
| '181 operand → F | ~30 ns |
...
2. **The register file contract makes this module the timing owner** on
   ...
   clock-high phase is this board's budget — and it now includes U20's
...
bus goes through U7 where the host can arbitrate. When /AUXOE is low the aux
GAL owns the B port — one driver per net is the host's discipline, the board
does not arbitrate. U9, U12, U13 must be HCT (they read LS outputs); U7, U8,
U10 may be plain HC (they read U9). The '182 is Schottky — decouple it
...
2. **GAL1/GAL3 split on the Addy Fetch & Control board.** With S/M/Cn decode
   moved into U20, GAL3's remit shrinks — possibly to nothing. Revisit the
...
4. **Header pinout ratification** — pending the TTLSim export, as with every
   board in the family.
5. **Flag latch spares** — thirteen unused flip-flops in U14/U15; pad field
   vs header.
...
U20 output rather than a host wire), the aux GAL socket, parity
