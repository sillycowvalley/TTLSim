...
enabled — the ROM data nets are private, point-to-point into the register
D-inputs, and leave the board only through the register stage.
...
U3/U4 accept any of the '373/'374/'377 family (identical 20-pin footprint):
...
Defaults with nothing driving H5 (pullups win): '377 holds its last word and
drives it; '374 and '373 are silent (three-stated). Design rule embodied:
internal nets fail toward working (ROM outputs enabled), bus-facing controls
fail toward quiet.
...
**Timing contract.** /EN is sampled by the clock edge with setup/hold like a
D input — it must be stable before the rising edge, not merely "low
sometime during the state." Address-to-load: the word is fetchable when the
address has been stable for the EEPROM access time plus register setup
...
precisely so that a runaway PC into blank memory freezes the machine rather
than executing noise; other hosts inherit whatever their all-ones word
means and should choose it as carefully.

## A15 and capacity

The 28C256 decodes A[14:0]: 32 K words. A15 arrives at H2.8, is not decoded
on-board, and exists for hosts with bank-select ambitions. With nothing
driving it, fit the H6 shunt (A15 → GND) so the pin is defined; **remove
the shunt before any host drives A15** — it is a hard short to ground
otherwise, and the header should be labelled so.
...
are hard-wired here. 64 K support means per-socket jumpers on pins 1 and 27
in a future revision; recorded so nobody discovers it with a programmer in
hand.
