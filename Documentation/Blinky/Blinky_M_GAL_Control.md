...
```
   opcode[7:4] ──►┌────────┐ bank enables
                  ...
                  └────────┘              │
                                          ▼
   opcode[3:0] ─┐        ┌─────────────────────────────┐
   T[2:0]  ─────┼───────►│  STAGE 1 — SEQUENCER GALs    │
   COND    ─────┘        │  one 22V10 per active family │
                         │  + one for interrupt entry   │
                         └───────────────┬──────────────┘
                                         │ IDX[6:0]  micro-op index
                                         │ /TRST     (registered, straight to T-counter)
                                         ▼
                         ┌─────────────────────────────┐
                         │  STAGE 2 — DECODE GALs       │
                         │  UOPA + UOPB (2× 22V10)      │
...
### Stage 1 — the sequencer (per-family)

A `'138` decodes `opcode[7:4]` into one enable per family. Each active family owns a
registered 22V10 whose inputs are `opcode[3:0]`, `T[2:0]`, and `COND` (8 lines), and
whose outputs are the 7-bit micro-op index plus `/TRST`. Because the family is fixed
within a bank, the high opcode nibble is not an input — it selected the chip — so each
sequencer solves a small 8-input problem.

The 22V10's **registered macrocells hold the T-state themselves**: the sequencer's own
next-state feedback replaces the discrete '161 T-counter, and `/TRST` becomes an
output equation (assert to reload T to zero on the next edge) rather than a stored ROM
bit feeding an external counter. Interrupt entry, being opcode-independent, is one
additional 22V10 enabled by `INTP` with `T` as its only live input.
...
The trade to weigh at ratification: an instruction or T-state change is a re-fit of the
affected sequencer GAL rather than an EEPROM burn, and a genuinely new micro-op (a
control-line combination not among the 57) re-fits the two decode GALs. Both are
regenerated by `BlinkyMGen` from the same instruction table that produces the microcode
today — a `--plds` target alongside the Intel HEX — so the fuse maps cannot drift from
the design. The verification chain is the established one: BlinkyJED compile, WinCUPL
fuse cross-check, then the TTLSim sweep.

---

## 8. Open items

1. **Per-family sequencer term counts are unmeasured.** Stage 2 is proven to fit; Stage
   1's index functions (8 inputs each) are very likely one 22V10 per family but need the
   generator's statistics pass extended to confirm, family by family. A bank that
   overflows takes a second GAL rather than failing.
2. **Registered-macrocell T-state feedback** needs the sequencer equations written in
   registered form and the reset path defined (async clear to T0 on power-up).
3. **Ratification of the fold set** (§3): PCINC derivation, ICLR-as-DST, PCBYTE-as-SRC,
   AMODE consolidation, and TOS-load-implied. Each is independently reversible at the
   cost of one control line.
4. **`BlinkyMGen --plds` emitter** to make both stages a generated, verified target of
   the existing C# source of truth.
