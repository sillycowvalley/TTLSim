...
// ====================================================== banked port I/O =====
// An alternative to the unrolled accessors above, for when SIMULTANEITY
// matters more than throughput.
//
// NOT AN OPTIMISATION. Measured on this board, write+read pair:
//     unrolled accessors : 158 ns
//     banked             : 226 ns
// The unrolled versions win because digitalWriteFast/digitalReadFast with a
// compile-time-constant pin fold to a single store or load, constant address
// and constant mask, on GPIO6-9 - the CPU-bus fast ports, one or two cycles
// each. There is no peripheral-bus penalty to remove, and the banked path
// replaces those folded constants with runtime table lookups.
//
// What it does buy: the unrolled writer issues eight separate stores, so
// bit 0 changes roughly 70 ns before bit 7. Banked changes every bit within
// a GPIO bank on a single store. As a coupling aggressor against a DUT with
// a clock or strobe in the loom, that is materially harsher and much closer
// to a real bus turnaround. Use it when stressing signal integrity; use the
// unrolled accessors for everything else.
//
...
// the mapping above ever changes. Note both the set/clear and input register
